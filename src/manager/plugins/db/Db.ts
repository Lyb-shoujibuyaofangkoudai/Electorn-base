...
import { V_2 } from "./migrations/v2.0";
...
export class Db implements IPlugin {
    ...
    try {
      this._dbPath =
        import.meta.env.MODE === "production"
          ? join(app.getPath("userData"), import.meta.env.VITE_DB_NAME)
          : join(
              app.getAppPath(),
              `/src/manager/plugins/db/${import.meta.env.VITE_DB_NAME}`,
            );
  ...
  private async _recreateDatabase(dataSource: DataSource, dbPath: string) {
    ...
    if (existsSync(dbPath)) {
      const backupPath = join(
        dbPath,
        `../${dayjs().format("YYYYMMDDHHmmssSSS")}_bk.db`,
      );
  ...
  private async _performUpgrades(r: QueryRunner, currentVersion: number) {
    ...
    this._logger?.info(
      `即将进行的数据库升级数量: ${pendingUpgrades.length}`,
      LOGGER_NAMESPACE.APP,
    );
