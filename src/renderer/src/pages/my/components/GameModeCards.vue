        ...
        <n-card :class="[
          ...
          <template #header>
            <div class="px-4 h-16 font-bold flex items-center justify-between">
              ...
              <div v-if="mode.isRanked" class="flex flex-col items-center">
                <img :src="mode.rankIcon" class="w-8 h-8" :alt="mode.rankInfo?.tier || 'Unranked'">
                <div class="text-xs text-[var(--text-color-2)] mt-1">
                  {{ getRankText(mode.rankInfo) }} {{ mode.rankInfo?.leaguePoints || 0 }} LP
                </div>
              </div>
            </div>
          </template>
          ...
          <div class="px-4 flex flex-col gap-4">
            ...
            <div class="text-sm text-[var(--text-color-2)]">
              近 <span class="text-[#3498DB] font-bold">{{ selectedGameCount }}</span> 场{{ mode.title }}
            </div>
                ...
                <div class="flex flex-col gap-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-[var(--text-color-2)]">
                      胜场: {{ mode.isRanked ? mode.rankInfo?.wins || 0 : mode.stats?.wins || 0 }}
                    </span>
                  </div>
                  <div class="flex items-center gap-2">
                    <div class="w-2 h-2 rounded-full bg-[#E74C3C]"></div>
                    <span class="text-[var(--text-color-2)]">
                      负场: {{ mode.isRanked ? mode.rankInfo?.losses || 0 : mode.stats?.losses || 0 }}
                    </span>
                  </div>
                </div>
              </div>
...
watch(() => gameModes.value, (newModes) => {
  // 图表的更新现在由 ref 回调函数处理
}, { deep: true })
