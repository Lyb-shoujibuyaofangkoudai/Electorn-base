 ...
 *
 * @param {NTheme.Config} config store themeConfig
 * @param neutralThemeConfig
 * @param {boolean} darkMode 暗黑模式
 * @return {ThemeColor}
 */
export function getThemeColors(
  config: NTheme.Config,
  neutralThemeConfig: NTheme.NeutralThemeType,
  darkMode: boolean
): ThemeColor {
  ...
  Object.entries(neutralThemeConfig).forEach(([key, value]) => {
    const { color, effects } = value as NTheme.NeutralThemeItemType
    effects.forEach((effect) => {
      const colorKey = `${effect}` as ColorKey
      themeColor[colorKey] = color
    })
  })
  return themeColor
...
export function getThemeOverrides(
  ...
  const themeColors = getThemeColors(config, neutralThemeConfig,darkMode)
