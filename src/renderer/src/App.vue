<template>
  <n-message-provider>
    ...
    >
      ...
      <!--    <n-theme-editor>-->
      <NScrollbar
        :class="`bg-c_wc-top ${!windowInfoStore.isMaxWindow ? 'rounded-xl' : ''}`"
      >
        <HomeLayout>
...
const api = useApi();
const { theme, themeOverrides, themeConfig, neutralThemeConfig } = storeToRefs(
  themeStore
);
...
function initTheme() {
  ...
  if (neutralThemeConfig)
    themeStore.setNeutralThemeConfig(neutralThemeConfig as NTheme.NeutralThemeType);
}

getLeagueInfo();

async function getLeagueInfo() {
  ipc.onEvent(EVENT_TYPE.SET_LOL_DETAILS, (data) => {
...
async function getSomeInfo() {
  try {
    ...
    await requestDataStore.fetchData(
      "summoner",
      async () => await api.lcuApi.summoner.getCurrentSummoner()
    );
    ...
    await requestDataStore.fetchData(
      "ranked",
      async () => await api.lcuApi.ranked.getCurrentRankedStats()
    );
  } catch (e) {
...
async function getLocalRiotSgpServerInfo() {
  const res = await ipc.call(EVENT_TYPE.LOCAL_SERVER_DETAILS);
